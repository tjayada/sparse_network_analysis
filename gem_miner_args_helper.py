...
class ArgsHelper:
    ...
    def parse_arguments(self, jupyter_mode=False):
        ...
        parser.add_argument("--experiment", default="None",
                            help="Experiment name, either None (default) or gem_miner")
        ...
        parser.add_argument("--submask-size", type=int, default=1, metavar="S",
                            help="Size of random 0/1 submask to create among a random set of weights in the network (default: 1)")
        parser.add_argument("--metric", type=str, default="loss", metavar="M",
                            help="Metric used to determine whether a weight or activation should be pruned (default: \"loss\")")
        parser.add_argument("--pruning-strategy", type=str, default=None, metavar="PS",
        ...
        parser.add_argument("--how-to-prune", type=str, default="random",
        ...
        parser.add_argument("--start-from-nothing", action="store_true", default=False,
        ...
        parser.add_argument("--flips", type=list, default=None, nargs="+", metavar="R",
        ...
        parser.add_argument("--lr-policy",type=str,default=None,
                            help="Learning rate scheduler")
        parser.add_argument("--lr-gamma",type=float,default=0.1,
                            help="Multistep lr decay ratio (default: 0.1)")
        parser.add_argument("--lr-adjust",type=int,default=50,
        ...
        parser.add_argument("--nesterov",type=bool,default=False,metavar="N",
                            help="Nesterov acceleration (default: False)")
        parser.add_argument("--milestones",type=list,default=[50, 100, 150, 200],nargs="+",metavar="M",
        ...
        parser.add_argument("--td",type=float,default=0.99,metavar="TD",
                            help="Temperature decay constant for simulated annealing (default: 0.99)")
        parser.add_argument("--temp",type=int,default=100000,metavar="T",
                            help="Temperature used in simulated annealing (default: 100000)")
        parser.add_argument("--max-iter",type=int,default=100000,metavar="MI",
        ...
        parser.add_argument("--iter-period", type=int,default=5,
        ...
        parser.add_argument("--score-init",type=str,default="unif",
                            help="initial score for hypercube |unif|bern|")
        parser.add_argument("--interpolate",type=str,default="prob",
                            help="way of interpolating masks/weights |prob|linear|")
        parser.add_argument('--plot-hc-convergence',action='store_true',default=False,
                            help='flag that decides if we plot convergence of hc')
        parser.add_argument("--hc-warmup",default=9999,type=int,
                            help="warmup epochs for hypercube")
        parser.add_argument("--hc-period",default=1,type=int,
        ...
        parser.add_argument("--arch",type=str, default="TwoLayerFC")
        ...
        parser.add_argument("--freeze-weights",action="store_true",default=True,
        ...
        parser.add_argument("--dense-training",action="store_true",
        ...
        parser.add_argument("--score-init-constant",type=float,default=None,
        ...
        # add for bottom K iterative pruning
        parser.add_argument("--prune-type",default="FixThresholding", type=str,
        ...
        parser.add_argument("--loss",type=str,default="cross-entropy-loss",
                            help="which loss to use for pruning: cross-entropy-loss or zero-one-loss")
        #    # Save/Load
        #parser.add_argument("--save-dir",type=str,default=False,
        #                   help="Directory to save the results of the experiment (default: \"./results\")")
        parser.add_argument("--save-plot-data",action="store_true",default=False,
        ...
        parser.add_argument("--seed-fixed-init",type=int,default=24,metavar="S",
        ...
        parser.add_argument("--fixed-init",action="store_true",default=False,
                            help="fixed weight initialization")
        parser.add_argument("--mode-connect",action="store_true",default=False,
                            help="Boolean flag to indicate whether to run mode connectivity")
        parser.add_argument('--mode-connect-filename',type=str,default=None,
                            help='filename for state_dict used for mode connectivity')
        parser.add_argument('--how-to-connect',type=str,default='prob',
        ...
        parser.add_argument("--pretrained2",dest="pretrained2",default=None,type=str,
                            help="use pre-trained model 2")
        parser.add_argument("--save_every",default=-1,type=int,
                            help="Save every ___ epochs")
        parser.add_argument("-e","--evaluate",dest="evaluate",action="store_true",
                            help="evaluate model on validation set",)
        parser.add_argument("--compare-rounding",dest="compare-rounding",action="store_true",
        ...
        parser.add_argument('--weight-training',action='store_true',default=False,
        ...
        parser.add_argument("--random-subnet",action="store_true",default=False,
                            help="Just initializes random subnetwork and then trains")
        parser.add_argument("--hc-quantized",action="store_true",default=False,
                            help="round probablities in every iteration")
        parser.add_argument("--quantize-threshold",default=0.5,type=float,
                            help="threhsold to use while quantizing scores in HC")
        parser.add_argument("--checkpoint-at-prune",action="store_true",default=False,
        ...
        parser.add_argument("--chg_mask",action="store_true",default=False,
                            help="chg masks before sanity check")
        parser.add_argument("--chg_weight",action="store_true",default=False,
                            help="chg weights before sanity check")
        parser.add_argument("--fine-tune-optimizer",type=str,default='sgd',
                            help="optimizer option to use |sgd|adam| for fine-tuning weights")
        parser.add_argument("--fine-tune-lr-policy",type=str,default=None,
                            help="Learning rate scheduler (for finetune)")
        parser.add_argument("--fine-tune-lr",type=float,default=0.01,metavar="LR",
                            help="Learning rate for fine-tuning weights")
        parser.add_argument("--fine-tune-wd",type=float,default=0.0001,metavar="WD",
                            help="Weight decay for fine-tuning weights")
        #parser.add_argument("--multigpu",default=None,type=str,
        #                   help="Which GPUs to use for multigpu training, comma separated")
        parser.add_argument("--rewind-score",action="store_true",default=False,
                            help="if set True, every time when we prune, we set the score back to the initial state")
        parser.add_argument("--rewind-to-epoch",default=-1,type=int,
        ...
        parser.add_argument("--differentiate-clamp",action="store_true",default=False,
                            help="if set True, we project to [0, 1] in forward and therefore differentiate clamp ")
        parser.add_argument("--project-freq",default=1,type=int,
                            help="project scores to [0, 1] every k gradient steps")
        parser.add_argument("--run_idx",default=None,
        ...
        parser.add_argument("--imp_rewind_iter", default=1000, type=int, 
                            help="which iterations to rewind to")
        parser.add_argument("--imp-resume-round",default=-1,type=int, 
                            help="which round to resume to")
        parser.add_argument("--imp-resume-epoch",default=-1,type=int, 
                            help="which epoch to resume to")
        parser.add_argument("--imp-resume-iter",default=-1,type=int, 
                            help="which iter to resume to")
        parser.add_argument("--imp-rewind-model", 
                            default="short_imp/Liu_checkpoint_model_correct.pth")
        parser.add_argument("--imp-no-rewind",action="store_true",default=False,
                            help="if set True, we run IMP algorithm without rewinding to previous states")
        parser.add_argument("--imp-rounds",type=int,default=-1,
                            help="if set > 0, then ignore the epochs statement, and calculate epochs based on rounds * iter / round")
        parser.add_argument("--smart_ratio", type=float,default=-1,
                            help="the pruning weights in [0, 1]. E.g. smart_ratio = 0.98 will end up with a 2\% weight remaining model")
        parser.add_argument("--bottom-k-on-forward",action="store_true",default=False,
        ...
        parser.add_argument("--lam_finetune_loss",type=float,default=-1,
                            help="lambda for finetune loss ")
        parser.add_argument("--num_step_finetune",type=int,default=10,
                            help="number of steps to check finetune loss ")
        parser.add_argument("--unflag-before-finetune",action="store_true",default=False,
                            help="Enable this to unprune weights if possible, before fine-tune")
        parser.add_argument("--override-prune-rate",action="store_true",default=False,
        ...
        parser.add_argument("--only-sanity",action="store_true",default=False,
                            help="Only run sanity checks on the files in specific directory or subdirectories")
        parser.add_argument("--invert-sanity-check",action="store_true",default=False,
                            help="Enable this to run the inverted sanity check (for HC)")
        parser.add_argument("--sanity-folder",default=None,type=str,metavar="PATH",
                            help="directory(s) to access for only sanity check",)
        parser.add_argument("--sr-version",default=1,type=int,
                            help="smart ratio version number (1, 2, ...)")
        parser.add_argument("--use-full-data",action="store_true",default=None,
                            help="Enable this use full train data and not leave anything for validation")
        parser.add_argument("--ckpt-at-fixed-epochs",default=None,type=str,
                            help="pass a list of epochs to load ckpt from later. Mainly for warm-GM")
        parser.add_argument("--drop-bottom-half-weights",action="store_true",default=False,
    ...
    def get_config(self, gem_miner_parser_args, jupyter_mode=False):
        # get commands from command line
        override_args = _parser.argv_to_vars(sys.argv)

        # load yaml file
        yaml_txt = open(gem_miner_parser_args.config).read()
    ...
    def get_args(self, jupyter_mode=False):
        ...
        gem_miner_parser_args = self.parse_arguments(jupyter_mode)
