...
def get_layers(arch='Conv4', model=None):
    ...
    if arch == 'Conv4':
        ...
        #conv_layers = [model.model[1], model.model[11]]
        conv_layers = []
        linear_layers = [model.model[1], model.model[3], model.model[5], model.model[7], model.model[9], model.model[11]]
...
def redraw(model, shuffle=False, reinit=False, invert=False, chg_mask=False, chg_weight=False):
    ...
    conv_layers, linear_layers = get_layers(gem_miner_parser_args.arch, cp_model)
...
def save_checkpoint(state, is_best, filename="checkpoint.pth", save=False, gem_miner_parser_args=None):
    ...
    if is_best:
        ...
        if gem_miner_parser_args is not None and gem_miner_parser_args.mode_connect:
            print('We are saving stat_dict for checking mode connectivity: {}'.format(
                gem_miner_parser_args.mode_connect_filename))
            shutil.copyfile(filename, gem_miner_parser_args.mode_connect_filename)
        ...
        if ".score" in name:
            ...
            if round_scheme == 'naive':
                params.data = torch.gt(params.data, torch.ones_like(
                    params.data)*gem_miner_parser_args.quantize_threshold).int().float()
...
def prune(model, update_thresholds_only=False, update_scores=False, drop_bottom_half_weights=False):
    ...
    if gem_miner_parser_args.algo not in ['hc_iter', 'global_ep', 'global_ep_iter']:
        print('Warning: Using prune() in {}. Are you sure?'.format(gem_miner_parser_args.algo))

    conv_layers, linear_layers = get_layers(gem_miner_parser_args.arch, model)

    if gem_miner_parser_args.prune_type == 'FixThresholding':
        if gem_miner_parser_args.algo == 'hc_iter':  # and update_thresholds_only == False:
        ...
        agg_biases = torch.cat(
            active_bias_list) if gem_miner_parser_args.bias else torch.tensor([])
        ...
        weight_threshold = torch.sort(
            torch.abs(agg_weights), descending=gem_miner_parser_args.invert_sanity_check).values[number_of_weights_to_prune-1].item()

        if gem_miner_parser_args.bias:
            bias_threshold = torch.sort(
                torch.abs(agg_bias_scores), descending=gem_miner_parser_args.invert_sanity_check).values[number_of_biases_to_prune-1].item()
        else:
            bias_scores_threshold = -1

        for layer in (conv_layers + linear_layers):
            if gem_miner_parser_args.invert_sanity_check:
    ...
    # prune the bottom k of scores.abs()
    elif gem_miner_parser_args.prune_type == 'BottomK':
        num_active_weights = 0
        num_active_biases = 0
        active_scores_list = []
        ...
        number_of_weights_to_prune = np.ceil(
            gem_miner_parser_args.prune_rate * num_active_weights).astype(int)
        number_of_biases_to_prune = np.ceil(
            gem_miner_parser_args.prune_rate * num_active_biases).astype(int)

        agg_scores = torch.cat(active_scores_list)
        agg_bias_scores = torch.cat(
            active_bias_scores_list) if gem_miner_parser_args.bias else torch.tensor([])

        # if invert_sanity_check, then threshold is based on sorted scores in descending order, and we prune all scores ABOVE it
        scores_threshold = torch.sort(
            torch.abs(agg_scores), descending=gem_miner_parser_args.invert_sanity_check).values[number_of_weights_to_prune-1].item()
        ...
        if update_thresholds_only:
            ...
            if gem_miner_parser_args.rewind_score and layer.saved_scores is not None:
    ...
    # prune the bottom k of scores.abs()
    elif gem_miner_parser_args.prune_type == 'LocalBottomK':
       for layer in (conv_layers + linear_layers):
            num_active_weights = 0
            num_active_biases = 0
            active_scores_list = []
...
def get_layer_sparsity(layer, threshold=0):
    if gem_miner_parser_args.algo in ['hc', 'hc_iter'] and not gem_miner_parser_args.bottom_k_on_forward:
    ...
    elif gem_miner_parser_args.algo in ['global_ep', 'ep', 'global_ep_iter'] or gem_miner_parser_args.bottom_k_on_forward:
        if gem_miner_parser_args.algo == 'ep':
            weight_mask, bias_mask = GetSubnetConv.apply(
                layer.scores.abs(), layer.bias_scores.abs(), gem_miner_parser_args.prune_rate)
        else:
...
# gem_miner_parser_args.prune_rate is a fraction i.e; 1/100*percentage
...
def get_prune_rate(target_sparsity=0.5, iter_period=5):
    ...
    max_epochs = gem_miner_parser_args.epochs
    num_prune_iterations = np.floor((max_epochs-1)/iter_period)
    # if algo is HC, iter_HC, global_EP, global_EP_iter or anything that uses prune()
    # then, prune_rate represents number of weights to prune
    prune_rate = 1 - np.exp(np.log(target_sparsity/100)/num_prune_iterations)
    # if algo is EP, then it represents target sparsity/100
    if gem_miner_parser_args.algo in ['ep']:
