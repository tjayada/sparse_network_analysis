...
class Builder(object):
    ...
    def conv(self, kernel_size, in_planes, out_planes, stride=1, first_layer=False, groups=1):
        ...
        if kernel_size == 3:
            conv = conv_layer(
                ...
                stride=stride,
                padding=1,
                bias=gem_miner_parser_args.bias,
                groups=groups
            )
        elif kernel_size == 1:
            conv = conv_layer(
                in_planes, out_planes, kernel_size=1, stride=stride, bias=gem_miner_parser_args.bias
            )
        elif kernel_size == 5:
            conv = conv_layer(
                in_planes,
    ...
    def linear(self, in_planes, out_planes):
        l = SubnetLinear(in_planes, out_planes, bias=gem_miner_parser_args.bias)
        self._init_conv(l)
        return l
    ...
    def activation(self):
        if gem_miner_parser_args.nonlinearity == "relu":
            return (lambda: nn.ReLU(inplace=True))()
        else:
            raise ValueError(f"{args.nonlinearity} is not an initialization option!")

    def _init_conv(self, conv):
        if gem_miner_parser_args.init == "signed_constant":

            fan = nn.init._calculate_correct_fan(conv.weight, gem_miner_parser_args.mode)
            if gem_miner_parser_args.scale_fan:
                fan = fan * (1 - gem_miner_parser_args.prune_rate)
            gain = nn.init.calculate_gain(gem_miner_parser_args.nonlinearity)
            std = gain / math.sqrt(fan)
            conv.weight.data = conv.weight.data.sign() * std

        elif gem_miner_parser_args.init == "unsigned_constant":
        ...
        elif gem_miner_parser_args.init == "kaiming_normal":
            ...
            if gem_miner_parser_args.scale_fan:
                ...
                nn.init.kaiming_normal_(
                    conv.weight, mode=gem_miner_parser_args.mode, nonlinearity=gem_miner_parser_args.nonlinearity
                )

        elif gem_miner_parser_args.init == "kaiming_uniform":
            nn.init.kaiming_uniform_(
                conv.weight, mode=gem_miner_parser_args.mode, nonlinearity=gem_miner_parser_args.nonlinearity
            )
        elif gem_miner_parser_args.init == "xavier_normal":
            nn.init.xavier_normal_(conv.weight)
        elif gem_miner_parser_args.init == "xavier_constant":

            fan_in, fan_out = nn.init._calculate_fan_in_and_fan_out(conv.weight)
            std = math.sqrt(2.0 / float(fan_in + fan_out))
            conv.weight.data = conv.weight.data.sign() * std

        elif gem_miner_parser_args.init == "standard":

            nn.init.kaiming_uniform_(conv.weight, a=math.sqrt(5))

        else:
            raise ValueError(f"{gem_miner_parser_args.init} is not an initialization option!")
...
def get_builder():
    ...
    if gem_miner_parser_args.first_layer_type is not None:
        first_layer = getattr(Utils.conv_type, gem_miner_parser_args.first_layer_type)
        print(f"==> First Layer Type: {gem_miner_parser_args.first_layer_type}")
    else:
        first_layer = None
