...
from Utils.SmartRatio import SmartRatio
...
def print_layers(gem_miner_parser_args, model):
...
def print_model(model, gem_miner_parser_args):
    #from torchsummary import summary
    #summary(model.cuda(), (3,32,32)) # for cifar
    # check the model architecture
    
    num_params = 0
    if gem_miner_parser_args.algo == 'training':
...
def do_sanity_checks(model, gem_miner_parser_args, data, criterion, epoch_list, test_acc_before_round_list, test_acc_list, val_acc_list, train_acc_list,
    ...
    cp_model = finetune(cp_model, gem_miner_parser_args, data, criterion, epoch_list, test_acc_before_round_list, test_acc_list, val_acc_list, train_acc_list,
                        reg_loss_list, model_sparsity_list, result_root, reinit=True, chg_weight=True)

    '''
    print("Sanity Check 2: Weight Reshuffle")
    cp_model = copy.deepcopy(model)
    cp_model = finetune(cp_model, gem_miner_parser_args, data, criterion, epoch_list, test_acc_before_round_list, test_acc_list,
...
def save_checkpoint_at_prune(model, gem_miner_parser_args):
...
def evaluate_without_training(gem_miner_parser_args, model, model2, validate, data, criterion):
    if gem_miner_parser_args.algo in ['hc_iter']:
        model = round_model(model, gem_miner_parser_args.round, noise=gem_miner_parser_args.noise,
                            ratio=gem_miner_parser_args.noise_ratio, rank=gem_miner_parser_args.gpu)
        eval_and_print(validate, data.val_loader, model, criterion, gem_miner_parser_args, writer=None,
                       epoch=gem_miner_parser_args.start_epoch, description='final model after rounding')
    for trial in range(gem_miner_parser_args.num_test):
        if gem_miner_parser_args.algo in ['hc']:
            if gem_miner_parser_args.how_to_connect == "prob":
                cp_model = round_model(model, gem_miner_parser_args.round, noise=gem_miner_parser_args.noise,
                                       ratio=gem_miner_parser_args.noise_ratio, rank=gem_miner_parser_args.gpu)
            else:
                cp_model = copy.deepcopy(model)
            eval_and_print(validate, data.val_loader, cp_model, criterion, gem_miner_parser_args,
                           writer=None, epoch=gem_miner_parser_args.start_epoch, description='model after pruning')
    if gem_miner_parser_args.pretrained2:
        eval_and_print(validate, data.val_loader, model2, criterion, gem_miner_parser_args,
                       writer=None, epoch=gem_miner_parser_args.start_epoch, description='model2')
        if gem_miner_parser_args.algo in ['hc']:
            if gem_miner_parser_args.how_to_connect == "prob":
                cp_model2 = round_model(model2, gem_miner_parser_args.round, noise=gem_miner_parser_args.noise,
                                        ratio=gem_miner_parser_args.noise_ratio, rank=gem_miner_parser_args.gpu)
            else:
                cp_model2 = copy.deepcopy(model2)
            eval_and_print(validate, data.val_loader, cp_model2, criterion, gem_miner_parser_args,
                           writer=None, epoch=gem_miner_parser_args.start_epoch, description='model2 after pruning')
    if gem_miner_parser_args.pretrained and gem_miner_parser_args.pretrained2 and gem_miner_parser_args.mode_connect:
        ...
        elif gem_miner_parser_args.algo in ['hc', 'ep', 'global_ep']:
...
#def test_smart_ratio(model, data, criterion, gem_miner_parser_args, result_root):
  


def test_random_subnet(model, data, criterion, gem_miner_parser_args, result_root, smart_ratio=-1):
    ...
    if smart_ratio != -1:
        ...
        model = SmartRatio(model, smart_ratio_args, gem_miner_parser_args)
        ...
        model = round_model(model, gem_miner_parser_args.round, noise=gem_miner_parser_args.noise, ratio=gem_miner_parser_args.noise_ratio, rank=gem_miner_parser_args.gpu)    
        
        # TODO: CHANGE THIS BACK once the finetune from checkpoints code is fixed
        # NOTE: this part is hard coded
        model = redraw(model, shuffle=gem_miner_parser_args.shuffle, reinit=gem_miner_parser_args.reinit, chg_mask=gem_miner_parser_args.chg_mask, chg_weight=gem_miner_parser_args.chg_weight)  
    ...
    model = finetune(model, gem_miner_parser_args, data, criterion, 
                    old_epoch_list, old_test_acc_before_round_list, old_test_acc_list, old_val_acc_list, old_train_acc_list, old_reg_loss_list, old_model_sparsity_list, 
...
def eval_and_print(validate, data_loader, model, criterion, gem_miner_parser_args, writer=None, epoch=gem_miner_parser_args.start_epoch, description='model'):

    acc1, acc5, acc10 = validate(
        data_loader, model, criterion, gem_miner_parser_args, writer=None, epoch=gem_miner_parser_args.start_epoch)
...
def finetune(model, gem_miner_parser_args, data, criterion, old_epoch_list, old_test_acc_before_round_list, old_test_acc_list, old_val_acc_list, old_train_acc_list, old_reg_loss_list, old_model_sparsity_list, result_root, shuffle=False, reinit=False, invert=False, chg_mask=False, chg_weight=False):
    ...
    if gem_miner_parser_args.bottom_k_on_forward:
        ...
        if gem_miner_parser_args.unflag_before_finetune:
            ...
            # round the score (in the model itself)
            model = round_model(model, round_scheme=gem_miner_parser_args.round, noise=gem_miner_parser_args.noise,
                                ratio=gem_miner_parser_args.noise_ratio, rank=gem_miner_parser_args.gpu)
            post_round_sparsity = get_model_sparsity(model)
    elif gem_miner_parser_args.algo in ['ep', 'global_ep', 'global_ep_iter']:
        post_round_sparsity = get_model_sparsity(model)
    ...
    gem_miner_parser_args.regularization = False
    ...
    optimizer = get_optimizer(gem_miner_parser_args, model, finetune_flag=True)
    scheduler = get_scheduler(optimizer, policy=gem_miner_parser_args.fine_tune_lr_policy)
    ''' 
    if gem_miner_parser_args.epochs == 150:
        scheduler = get_scheduler(optimizer, gem_miner_parser_args.fine_tune_lr_policy, milestones=[
                                  80, 120], gamma=0.1)  # NOTE: hard-coded
    elif gem_miner_parser_args.epochs == 50:
        scheduler = get_scheduler(optimizer, gem_miner_parser_args.fine_tune_lr_policy, milestones=[
                                  20, 40], gamma=0.1)  # NOTE: hard-coded
    elif gem_miner_parser_args.epochs == 200:
        scheduler = get_scheduler(optimizer, gem_miner_parser_args.fine_tune_lr_policy, milestones=[
                                  100, 150], gamma=0.1)  # NOTE: hard-coded
    elif gem_miner_parser_args.epochs == 300:
        scheduler = get_scheduler(optimizer, gem_miner_parser_args.fine_tune_lr_policy, milestones=[
    ...
    acc1, acc5, acc10 = validate(
        data.val_loader, model, criterion, gem_miner_parser_args, writer, gem_miner_parser_args.epochs-1)
    val_acc1, val_acc5, val_acc10 = validate(
        data.actual_val_loader, model, criterion, gem_miner_parser_args, writer, gem_miner_parser_args.epochs-1)
    train_acc1, train_acc5, train_acc10 = validate(
        data.train_loader, model, criterion, gem_miner_parser_args, writer, gem_miner_parser_args.epochs-1)
    avg_sparsity = post_round_sparsity
    epoch_list.append(gem_miner_parser_args.epochs-1)
    ...
    end_epoch = time.time()
    for epoch in range(gem_miner_parser_args.epochs, gem_miner_parser_args.epochs*2):
        ...
        train_acc1, train_acc5, train_acc10, reg_loss = train(
            data.train_loader, model, criterion, optimizer, epoch, gem_miner_parser_args, writer=writer
        )
        ...
        val_acc1, val_acc5, val_acc10 = validate(
            data.actual_val_loader, model, criterion, gem_miner_parser_args, writer, epoch)
...
def get_idty_str(gem_miner_parser_args):
    train_mode_str = 'weight_training' if gem_miner_parser_args.weight_training else 'pruning'
    dataset_str = gem_miner_parser_args.dataset
    model_str = gem_miner_parser_args.arch
    algo_str = gem_miner_parser_args.algo
    rate_str = gem_miner_parser_args.prune_rate
    period_str = gem_miner_parser_args.iter_period
    reg_str = 'reg_{}'.format(gem_miner_parser_args.regularization)
    reg_lmbda = gem_miner_parser_args.lmbda if gem_miner_parser_args.regularization else ''
    opt_str = gem_miner_parser_args.optimizer
    policy_str = gem_miner_parser_args.lr_policy
    lr_str = gem_miner_parser_args.lr
    lr_gamma = gem_miner_parser_args.lr_gamma
    lr_adj = gem_miner_parser_args.lr_adjust
    finetune_lr_str = gem_miner_parser_args.fine_tune_lr
    fan_str = gem_miner_parser_args.scale_fan
    w_str = gem_miner_parser_args.init
    s_str = gem_miner_parser_args.score_init
    width_str = gem_miner_parser_args.width
    seed_str = gem_miner_parser_args.seed + gem_miner_parser_args.trial_num - 1
    run_idx_str = gem_miner_parser_args.run_idx
    lam_ft_str = gem_miner_parser_args.lam_finetune_loss
    n_step_ft_str = gem_miner_parser_args.num_step_finetune
...
def get_settings(gem_miner_parser_args):

    run_base_dir, ckpt_base_dir, log_base_dir = get_directories(gem_miner_parser_args)
    gem_miner_parser_args.ckpt_base_dir = ckpt_base_dir
...
def compare_rounding(validate, data_loader, model, criterion, gem_miner_parser_args, result_root):
    ...
    for i in range(n_rand):
        ...
        acc1 = eval_and_print(validate, data_loader, prob_model, criterion,
                              gem_miner_parser_args, description='probabilistic model {}'.format(i))
        idx_list.append(i)
        test_acc_list.append(acc1)
...
def switch_to_wt(model):
    ...
    gem_miner_parser_args.lam_finetune_loss = 0
    for name, params in model.named_parameters():
        # make sure param_name ends with .weight or .bias
        if re.match('.*\.weight', name):
            params.requires_grad = True
        elif gem_miner_parser_args.bias and re.match('.*\.bias$', name):
            params.requires_grad = True
        elif "score" in name:
...
def connect_mask(model, criterion, data, validate, model2=None):
    ...
    for d1_idx in range(num_d):
        ...
        for i in range(resol+1):
            ...
            for v_idx in range(num_v):
                ...
                if gem_miner_parser_args.how_to_connect == "round":
                    cp_model = round_model(cp_model, gem_miner_parser_args.round, noise=gem_miner_parser_args.noise,
                                           ratio=gem_miner_parser_args.noise_ratio, rank=gem_miner_parser_args.gpu)

                # compute loss for the mask
                loss = criterion(cp_model(data_), label_)
                acc1, acc5, acc10 = validate(
                    data.val_loader, cp_model, criterion, gem_miner_parser_args,
                    writer=None, epoch=gem_miner_parser_args.start_epoch)

                train_acc1, train_acc5, train_acc10 = validate(
                    data.train_loader, cp_model, criterion, gem_miner_parser_args,
                    writer=None, epoch=gem_miner_parser_args.start_epoch)
    ...
    if model2 is None:
        results_filename = "results/results_visualize_sharpness_sparsity1_{}_d1_{}_v_{}_{}_{}_{}.csv".format(
            sparsity1, num_d, num_v, train_mode_str, gem_miner_parser_args.dataset, gem_miner_parser_args.algo)
    else:
        results_filename = "results/results_visualize_connectivity_d_{}_v_{}_resol_{}_{}_{}_{}_{}.csv".format(
            num_d, num_v, resol, train_mode_str, gem_miner_parser_args.dataset, gem_miner_parser_args.algo, gem_miner_parser_args.interpolate)
...
def connect_weight(model, criterion, data, validate, model2=None):
    ...
    if gem_miner_parser_args.interpolate == 'linear':
        num_v = 1
    ...
    for d1_idx in range(num_d):
        ...
        for i in range(resol+1):
            ...
            for v_idx in range(num_v):
                if gem_miner_parser_args.interpolate == 'prob':
                ...
                elif gem_miner_parser_args.interpolate == 'linear':
    ...
    if model2 is None:
        results_filename = "results/results_visualize_sharpness_sparsity1_{}_d1_{}_v_{}_{}_{}_{}_{}.csv".format(
            sparsity1, num_d, num_v, train_mode_str, gem_miner_parser_args.dataset, gem_miner_parser_args.algo, gem_miner_parser_args.interpolate)
    else:
        results_filename = "results/results_visualize_connectivity_d_{}_v_{}_{}_{}_{}_{}.csv".format(
            num_d, num_v, train_mode_str, gem_miner_parser_args.dataset, gem_miner_parser_args.algo, gem_miner_parser_args.interpolate)
...
def visualize_mask_2D(model, criterion, data, validate):
    ...
    results_filename = "results/results_2D_visualize_sharpness_epoch_sparsity_{}_d_{}_v_{}_{}_{}_{}".format(
        sparsity, num_d, num_v, train_mode_str, gem_miner_parser_args.dataset, gem_miner_parser_args.algo)
...
def get_trainer(gem_miner_parser_args):
    print(f"=> Using trainer from Trainers.{gem_miner_parser_args.trainer}")
    trainer = importlib.import_module(f"Trainers.{gem_miner_parser_args.trainer}")

    return trainer.train, trainer.validate, trainer.modifier


def set_gpu(gem_miner_parser_args, model):
    assert torch.cuda.is_available(), "CPU-only experiments currently unsupported"

    if gem_miner_parser_args.gpu is not None:
        torch.cuda.set_device(gem_miner_parser_args.gpu)
        model.cuda(gem_miner_parser_args.gpu)

        if gem_miner_parser_args.multiprocessing_distributed:
            torch.distributed.init_process_group(
                backend=gem_miner_parser_args.dist_backend,
                init_method='env://',
                world_size=gem_miner_parser_args.world_size,
                rank=gem_miner_parser_args.rank
            )
            model = nn.parallel.DistributedDataParallel(
                model, device_ids=[gem_miner_parser_args.gpu], find_unused_parameters=True)
    else:
...
def resume(gem_miner_parser_args, model, optimizer):
    if os.path.isfile(gem_miner_parser_args.resume):
        print(f"=> Loading checkpoint '{gem_miner_parser_args.resume}'")

        checkpoint = torch.load(
            gem_miner_parser_args.resume, map_location=f"cuda:{gem_miner_parser_args.gpu}")
        #if gem_miner_parser_args.start_epoch is None:
        #    print(f"=> Setting new start epoch at {checkpoint['epoch']}")
        #    parsegem_miner_parser_argsr_args.start_epoch = checkpoint["epoch"]
        ...
        #print(
        #    f"=> Loaded checkpoint '{gem_miner_parser_args.resume}' (epoch {checkpoint['epoch']})")

        return 0
    else:
        print(f"=> No checkpoint found at '{gem_miner_parser_args.resume}'")
...
def pretrained(path, model):
    if os.path.isfile(path):
        ...
        model.load_state_dict(torch.load(
            path, map_location=torch.device("cuda:{}".format(gem_miner_parser_args.gpu))))
        model.eval()
        '''
        pretrained = torch.load(path, map_location=torch.device("cuda:0"))["state_dict"]                 #map_location=torch.device("cuda:{}".format(gem_miner_parser_args.multigpu[0])),
...
def get_dataset(gem_miner_parser_args):
    print(f"=> Getting {gem_miner_parser_args.dataset} dataset")
    dataset = getattr(Data, gem_miner_parser_args.dataset)(gem_miner_parser_args)

    return dataset


def get_model(gem_miner_parser_args):
    if gem_miner_parser_args.first_layer_dense:
        gem_miner_parser_args.first_layer_type = "DenseConv"

    print("=> Creating model '{}'".format(gem_miner_parser_args.arch))
    if gem_miner_parser_args.fixed_init:
        set_seed(gem_miner_parser_args.seed_fixed_init)
    if gem_miner_parser_args.arch in ['Conv4', 'Conv4Normal']:
        model = Models.__dict__[gem_miner_parser_args.arch](width=gem_miner_parser_args.width)
    else:
        if gem_miner_parser_args.dataset == 'MNIST':
            input_shape, num_classes = (1, 28, 28), 10
        elif gem_miner_parser_args.dataset == 'CIFAR10':
            input_shape, num_classes = (3, 32, 32), 10
        if gem_miner_parser_args.arch == "FC":
            model = Models.__dict__[gem_miner_parser_args.arch](input_shape, num_classes)
        else: 
            model = Models.__dict__[gem_miner_parser_args.arch]()
    if gem_miner_parser_args.fixed_init:
        set_seed(gem_miner_parser_args.seed)

    if not gem_miner_parser_args.weight_training:
        # applying sparsity to the network
        if (
            gem_miner_parser_args.conv_type != "DenseConv"
            and gem_miner_parser_args.conv_type != "SampleSubnetConv"
            and gem_miner_parser_args.conv_type != "ContinuousSparseConv"
        ):
            if gem_miner_parser_args.prune_rate < 0:
                raise ValueError("Need to set a positive prune rate")

            set_model_prune_rate(model, prune_rate=gem_miner_parser_args.prune_rate)
            print(
                f"=> Rough estimate model params {sum(int(p.numel() * (1-gem_miner_parser_args.prune_rate)) for n, p in model.named_parameters() if not n.endswith('scores'))}"
            )

        # freezing the weights if we are only doing subnet training
        if gem_miner_parser_args.freeze_weights:
...
def get_directories(gem_miner_parser_args):
    if gem_miner_parser_args.config is None or gem_miner_parser_args.name is None:
        raise ValueError("Must have name and config")

    config = pathlib.Path(gem_miner_parser_args.config).stem
    if gem_miner_parser_args.log_dir is None:
        run_base_dir = pathlib.Path(
            f"runs/{config}/{gem_miner_parser_args.name}/prune_rate={gem_miner_parser_args.prune_rate}"
        )
    else:
        run_base_dir = pathlib.Path(
            f"{gem_miner_parser_args.log_dir}/{config}/{gem_miner_parser_args.name}/prune_rate={gem_miner_parser_args.prune_rate}"
        )
    if gem_miner_parser_args.width_mult != 1.0:
        run_base_dir = run_base_dir / \
            "width_mult={}".format(str(gem_miner_parser_args.width_mult))
    ...
    (run_base_dir / "settings.txt").write_text(str(gem_miner_parser_args))
