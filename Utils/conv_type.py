...
class GetSubnet(autograd.Function):
    ...
    def forward(ctx, scores, bias_scores, k, scores_prune_threshold=-np.inf, bias_scores_prune_threshold=-np.inf):
        if gem_miner_parser_args.algo == 'pt_hack':
            # Get the supermask by normalizing scores and "sampling" by probability
            if gem_miner_parser_args.normalize_scores:
        ...
        elif gem_miner_parser_args.algo == 'ep' or gem_miner_parser_args.algo == 'ep+greedy':
        ...
        elif gem_miner_parser_args.algo in ['global_ep', 'global_ep_iter']:
        ...
        elif gem_miner_parser_args.algo == 'pt':
            scores = torch.clamp(MULTIPLIER*scores, 0, 1)
            bias_scores = torch.clamp(MULTIPLIER*bias_scores, 0, 1)
            out = torch.bernoulli(scores)
            bias_out = torch.bernoulli(bias_scores)
            ...
            if gem_miner_parser_args.bottom_k_on_forward:
                ...
                out = torch.gt(scores, torch.ones_like(scores)*gem_miner_parser_args.quantize_threshold).float()
                bias_out = torch.gt(bias_scores, torch.ones_like(bias_scores)*gem_miner_parser_args.quantize_threshold).float()
...
class SubnetConv(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        if gem_miner_parser_args.algo in ['hc', 'hc_iter']:
            if gem_miner_parser_args.random_subnet:
                self.scores.data = torch.bernoulli(gem_miner_parser_args.prune_rate * torch.ones_like(self.scores.data))
                self.bias_scores.data = torch.bernoulli(gem_miner_parser_args.prune_rate * torch.ones_like(self.bias_scores.data))
            elif gem_miner_parser_args.score_init in ['half']:
                self.scores.data = 0.5 * torch.ones_like(self.scores.data)
                self.bias_scores.data = 0.5 * torch.ones_like(self.bias_scores.data)
            elif gem_miner_parser_args.score_init in ['bern']:
            ...
            elif gem_miner_parser_args.score_init in ['unif']:
                nn.init.uniform_(self.scores, a=0.0, b=1.0)
                nn.init.uniform_(self.bias_scores, a=0.0, b=1.0)
            elif gem_miner_parser_args.score_init in ['bimodal', 'skew']:
                Beta = torch.distributions.beta.Beta
                if gem_miner_parser_args.score_init == 'bimodal':
                    alpha, beta = 0.1, 0.1
                elif gem_miner_parser_args.score_init == 'skew':
                    alpha, beta = 1, 5
        ...
        if gem_miner_parser_args.rewind_score:
    ...
    def forward(self, x):
        if gem_miner_parser_args.algo in ['hc', 'hc_iter', 'transformer']:
            # don't need a mask here. the scores are directly multiplied with weights
            if gem_miner_parser_args.differentiate_clamp:
                self.scores.data = torch.clamp(self.scores.data, 0.0, 1.0)
                self.bias_scores.data = torch.clamp(self.bias_scores.data, 0.0, 1.0)

            if gem_miner_parser_args.hc_quantized:
                subnet, bias_subnet = GetSubnet.apply(self.scores, self.bias_scores, gem_miner_parser_args.prune_rate)
        ...
        elif gem_miner_parser_args.algo in ['imp']:
            ...
            subnet, bias_subnet = GetSubnet.apply(self.scores.abs(), self.bias_scores.abs(), gem_miner_parser_args.prune_rate)
        
        if gem_miner_parser_args.algo in ['imp']:
            # no STE, no subnet. Mask is handled outside
            w = self.weight
            b = self.bias
        else:
...
class SampleSubnetConv(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        self.scores = nn.Parameter(torch.Tensor(self.weight.size()))
        if gem_miner_parser_args.score_init_constant is not None:
            self.scores.data = (
                torch.ones_like(self.scores) * gem_miner_parser_args.score_init_constant
            )
        else:
            nn.init.kaiming_uniform_(self.scores, a=math.sqrt(5))
